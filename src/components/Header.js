import logo from "../../assets/images/crave-hub-logo.png";
...
export const Title = () => {
  return (
    <Link to='/'>
      {" "}
      <img
        className='logo ml-2.5 w-[50px] h-[50px] mt-2'
        alt={"logo"}
        src={logo}
      />{" "}
    </Link>
  ...
  return (
    <div className='flex justify-center items-center'>
      <span className='py-2.5 px-1 mt-2.5 mr-1 font-bold text-green'>
        {" "}
        {user ? `Welcome ${name} ` : "Please Login"} !!!
      </span>
    </div>
  );
};
...
export const Header = () => {
  const showProgressBar = useSelector(
    (store) => store.showProgressBar.showProgressBar
  );
  return (
